...
OMGHTTPURLRQ also makes it easy to send multipartForm POSTs:
...
`PromiseFulfiller` is `typedef void (^PromiseFulfiller)(id)`, i.e. a block that takes a parameter of `id` and returns `void`.
